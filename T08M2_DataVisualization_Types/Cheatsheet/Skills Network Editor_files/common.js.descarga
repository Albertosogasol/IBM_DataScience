            ...
            if (valid) {
                tag.context.innerHTML =
                    `<button class="plugin" 
                        ...
                        ${window.location.href.includes('edit') ? 'disabled style="cursor: not-allowed; opacity: 0.6;"' : ''}
                        title="${window.location.href.includes('edit') ? 'Only clickable for learners or for authors in the test mode' : ''}">
                        <i class="fa fa-gift"></i> 
                        <span class="plugin-text">Claim an <strong>IBM Cloud</strong> Token!</span>
                    </button>`
            }
            else {
                tag.remove()
...
function initTheme(themeTool, doc) {
    ...
    if (currentTheme === "dark") {
        themeTool.value = "off";
        themeTool.innerHTML = '<i class="fa fa-moon-o mr-1" aria-hidden="true"></i><i class="fa fa-toggle-off" aria-hidden="true"></i>';
        doc.setPreviewTheme('dark');
    } else {
        themeTool.value = "on";
        themeTool.innerHTML = '<i class="fa fa-sun-o mr-1" aria-hidden="true"></i><i class="fa fa-toggle-on" aria-hidden="true"></i>';
        doc.setPreviewTheme('default');
    }
}
