...
app.layout = html.Div(children=[
    ...
    dcc.Dropdown(
        ...
        options=[
            ...
            {'label': 'KSC LC-40', 'value': 'KSC LC-40'},
        ],
        value='ALL',
...
def get_pie_chart(selected_site):
    ...
    if selected_site == 'ALL':
        # If all sites are selected, show total success launches
        fig = px.pie(filtered_df, values='class', names='Launch Site', title='Total Success Launches')
        return fig
    else:
        # Filter for the selected site and show success vs failed counts
...
def get_scatter_plot(selected_site, payload_range):
    filtered_df = spacex_df
    if selected_site == 'ALL':
        # If all sites are selected, filter based on payload range
        filtered_df = filtered_df[
            (filtered_df['Payload Mass (kg)'] >= payload_range[0]) &
            (filtered_df['Payload Mass (kg)'] <= payload_range[1])
        ]
    else:
        # Filter for the selected site and based on payload range
        filtered_df = filtered_df[filtered_df['Launch Site'] == selected_site]
        filtered_df = filtered_df[
            (filtered_df['Payload Mass (kg)'] >= payload_range[0]) &
            (filtered_df['Payload Mass (kg)'] <= payload_range[1])
